...
var render = function() {
  ...
  var l0 = _vm.__map(_vm.cardsList, function(item, k) {
    var m0 = parseInt(
      ((item.use_end_time - _vm.time_now) / 1000 / 60 / 60) % 24
    )
    var m1 = parseInt(
      ((item.get_end_time - _vm.time_now) / 1000 / 60 / 60) % 24
    )
    var m2 = parseInt(((item.get_end_time - _vm.time_now) / 1000 / 60) % 60)
    var m3 = parseInt(((item.get_end_time - _vm.time_now) / 1000) % 60)
    return {
